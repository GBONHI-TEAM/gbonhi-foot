...
const SOUND_KEY = 'gbonhi_partner_sound_on';
...
/**
 * Alerte temps réel des réservations reçues sur les terrains du partenaire :
 * notification visuelle + alarme sonore répétée jusqu'à acquittement.
 */
export function ReservationAlerts() {
  const router = useRouter();
  const { terrains } = useTerrain();
  const terrainMap = useMemo(() => new Map(terrains.map((t) => [t.id, t.name])), [terrains]);
  ...
  useEffect(() => {
    ...
    const owned = new Set(terrains.map((t) => t.id));
    const channel = supabase
      .channel(`partner-resv-alerts-${Math.random().toString(36).slice(2)}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'reservations' }, (payload) => {
        const row = payload.new as ReservationRow;
        const tid = row.terrain_id;
        if (!tid || !owned.has(tid)) return; // uniquement mes terrains
        setAlerts((prev) => [
          {
            id: row.id,
            terrainName: terrainMap.get(tid) ?? 'Mon terrain',
            amount: row.total_price ?? null,
            date: row.reservation_date ?? null,
            slot: `${fmtHour(row.start_hour)}${row.end_hour != null ? ` – ${fmtHour(row.end_hour)}` : ''}`,
          },
          ...prev,
        ].slice(0, 20));
      })
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[60] flex flex-col items-end gap-2">
      ...
      {alerts.length > 0 && (
        <div className="w-80 overflow-hidden rounded-2xl border border-orange-200 bg-white shadow-2xl">
          <div className="flex items-center justify-between bg-[#F7921E] px-4 py-2.5 text-white">
            ...
            <button onClick={dismissAll} className="text-white/90 hover:text-white text-xs font-bold">Tout acquitter</button>
          </div>
          <ul className="max-h-72 divide-y divide-gray-100 overflow-y-auto">
            {alerts.map((a) => (
              <li key={a.id} className="flex items-center gap-3 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-gray-900 truncate">{a.terrainName}</p>
                  <p className="text-xs text-gray-500">{[a.amount != null ? `${a.amount.toLocaleString('fr-FR')} FCFA` : '', fmtDate(a.date), a.slot].filter(Boolean).join(' · ')}</p>
                </div>
