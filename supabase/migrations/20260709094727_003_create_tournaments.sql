...
CREATE POLICY "tournaments_organizer_insert" ON tournaments FOR INSERT WITH CHECK (auth.uid() = organizer_id);
CREATE POLICY "tournaments_organizer_update" ON tournaments FOR UPDATE USING (auth.uid() = organizer_id);
...
CREATE POLICY "tournament_teams_coach_register" ON tournament_teams
  FOR INSERT WITH CHECK (auth.uid() IN (SELECT coach_id FROM teams WHERE id = team_id));
